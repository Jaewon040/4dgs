...
def create_dummy_image(width, height, device='cuda'):
    """
    CUDA 장치에 특정 크기의 회색 이미지를 생성합니다.
    
    Args:
        width (int): 이미지 너비
        height (int): 이미지 높이
        device (str, optional): 이미지를 생성할 장치. 기본값은 'cuda'.
    
    Returns:
        torch.Tensor: 생성된 이미지 텐서
    """
    # 회색 이미지 생성 (3채널, 0.5 값)
    image = torch.ones(3, height, width, device=device) * 0.5
    return image

def edit_4dgs():
    # 명령줄 인자 파싱을 위한 주 파서 생성
    parser = argparse.ArgumentParser(description="4DGS 편집 스크립트")
    
    # 기존 모델 파라미터 파서 생성 (그러나 사용하지 않음)
    original_parser = argparse.ArgumentParser()
    model_params = ModelParams(original_parser)
    
    # 편집 관련 파라미터 추가
    parser.add_argument("--source_path", type=str, required=True, help="원본 모델 경로")
    ...
    parser.add_argument("--configs", type=str, help="설정 파일 경로")
    ...
    # 점프잭 모델용 하드코딩된 설정 (deformation.py의 요구사항을 충족)
    jumpingjacks_config = {
        'kplanes_config': {
            'grid_dimensions': 2,
    ...
    hyper_args = argparse.Namespace(**jumpingjacks_config)
    ...
    # Stable Diffusion InstructPix2Pix 모델 로드
    print("IP2P 모델 로드 중...")
    ...
    # 최적화 준비
    # 포인트 클라우드만 업데이트하고 변형 필드는 업데이트하지 않음
    print("최적화 설정 중...")
    l = [
